...
_imeiIndexes imeiIndexes[64];
int cntImeis = 0;
int currImei = 0;

bool getImeiMsg = false;
bool waitMsg = false;

char imei[16];

//Функция подсчета количества миллисекунд
...
void terminal_decode(char *parcel, int parcel_size, ST_ANSWER *answer, ST_WORKER *worker)
{
    ...
    if (head.msg.TYPE == 0x03)
    {
        ...
        memcpy(imei, imeiMessage.msg.IMEI, sizeof(imeiMessage.msg.IMEI));
        ...
        bool imeiFounded = false;
        for (int i = 0; i < cntImeis; i++)
        {
            if (strcmp(imeiIndexes[i].imei, imei) == 0)  {
                imeiFounded = true;
                break;
            }
        }
        if (!imeiFounded)
        {
            _imeiIndexes imeiIndex;
            memcpy(imeiIndex.imei, imei, sizeof(imeiIndex.imei));
            memcpy(imeiIndex.ip, ipAddress, sizeof(imeiIndex.ip));
            imeiIndexes[cntImeis] = imeiIndex;
            if (cntImeis < 64)
                cntImeis++;
        ...
        logging("terminal_decode[%s:%s:%d]: IMEI MSG: SIZE: %d DEV_ID: %X REPLY_EN: %X HW_TYPE: %s IMEI: %s\n", worker->listener->name, ipAddress, worker->listener->port, \
                                                                                        head.msg.SIZE, head.msg.DEVICE_ID, imeiMessage.msg.REPLY_ENABLED, hwType, imei);
        return;
    }
    ...
    if (head.msg.TYPE == 0x0A)
    {
        ...
        for (int i = 0; i < cntImeis; i++)
        {
            if (strcmp(imeiIndexes[i].ip, ipAddress) == 0)
            {
                memcpy(imei, imeiIndexes[i].imei, sizeof(imeiIndexes[i].imei));
                break;
            }
        }
